...
void ResidentialData::FormData(Blocks data)
{
  ...
  //for (unsigned int index = 0; index < 1000; index++)
  {
    ...
    /* In one of blocks, there is an extra line with ., remove that*/
    if (data_block.size() == 9)
    {
      data_block.erase(data_block.begin() + 4);
    }
    ...
    line_data = data_block.at(5);
    eng_data = Utilities::ConvertEnglish(line_data.at(5));
    eng_data = Utilities::ToFloat(eng_data);
    acc_details.total_cons_cost_ = eng_data.toFloat();

    line_data = data_block.at(6);
...
std::vector<AccountDetails> ResidentialData::GetDataInfo()
{
