...
{
  bool is_other_fees = Utilities::IsOtherFees();

  if (((is_other_fees) && (data_in.size() == TYPE_12_SMALL_BLOCK_LENGTH + 1))
      || ((!is_other_fees) && (data_in.size() == TYPE_12_SMALL_BLOCK_LENGTH)))
  {
    VatSmallParse(data_in, acc_details);
  } else
  {
    VatLargeParse(data_in, acc_details);
  }  
}
...
void Type12Parser::VatSmallParse(Block data_in, AccountDetails* acc_details)
{
  QStringList line_data;
...
void Type12Parser::VatLargeParse(Block data_in, AccountDetails* acc_details)
{
  QStringList line_data;
